...
def _check_limit(ip: str) -> tuple[bool, str]:
    ...
    if count >= FREE_LIMIT:
        return False, (
            "Free limit reached. "
            "<a href='/upgrade' class='link'>Unlock one more meld</a> ($3.33 one-time)."
        )
    return True, ""
...
<meta name="description" content="meld puts the context on a URL so neither side has to paste the block. Then the URL dies.">
...
.hero{font-size:1.65rem;font-weight:700;letter-spacing:-.02em;line-height:1.35;margin-bottom:.75rem;max-width:36rem}
...
<div class="nav"><a href="/">Create</a><a href="/trust">Trust</a><a href="/upgrade" id="nav-pro">Unlock</a></div>
</div>
...
function UP(){return'<div><div class="card" style="text-align:left"><h1>One price.<br>Per meld.</h1><p>No subscription, nothing recurring. Free tier, then $3.33 per meld.</p></div><div class="pg"><div class="pc"><div style="font-size:.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;color:var(--muted)">Free</div><div class="pr">$0</div><ul><li>3 melds per hour</li><li>1 hour expiry</li><li>E2E encryption option</li><li>No account, no card</li></ul><a href="/" class="btn btn-s" style="width:100%;justify-content:center">Get started</a></div><div class="pc f"><div style="font-size:.75rem;font-weight:600;text-transform:uppercase;letter-spacing:.05em;color:var(--success)">Per meld</div><div class="pr">$3.33 <span>one-time</span></div><ul><li>Unlocks one specific meld</li><li>Beyond the free tier</li><li>Stripe Checkout</li><li>Nothing recurring</li></ul><p style="margin-top:.75rem;font-size:.8125rem;color:var(--dim);text-align:center">Paid from the meld itself when the wall is hit — no cart, no account.</p></div></div><div class="card" style="text-align:center"><p>Stripe-powered. One-time payments only. There are no subscriptions.</p></div></div>'}\n\nfunction PR(){const s=new URLSearchParams(location.search).get('session_id')||'';if(s){localStorage.setItem('meld_pro','true')}return'<div class="card" style="text-align:center;padding:3rem 2rem"><div style="font-size:3rem;margin-bottom:.5rem">🎉</div><h1>Unlocked</h1><p style="max-width:400px;margin:.5rem auto 1.5rem;color:var(--dim)">One-time unlock applied. No subscription. Ephemeral by design.</p><a href="/" class="btn">Create a meld</a></div>'}
