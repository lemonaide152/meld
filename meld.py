...
async function _dec(blob,khex){if(!blob.startsWith('meld1:'))return blob;const k=await _key(khex);const bin=atob(blob.slice(6));const buf=new Uint8Array(bin.length);for(let i=0;i<bin.length;i++)buf[i]=bin.charCodeAt(i);const pt=await crypto.subtle.decrypt({name:'AES-GCM',iv:buf.slice(0,12)},k,buf.slice(12));return _td.decode(pt)}
...
async function R(){const e=document.getElementById('rb');const s=document.getElementById('rst');e.disabled=true;s.innerHTML='<span class="spin"></span>';try{let _ans=document.getElementById('ctxb').value;const _fk2=(location.hash.match(/k=([0-9a-f]+)/)||[])[1];if(_fk2&&_ans!==undefined){_ans=await _enc(_ans,_fk2)}
...
else if(p.startsWith('/m/')){_c=p.split('/m/')[1];const _ft=(location.hash.match(/t=([0-9a-f]{64})/)||[])[1];if(_ft&&!_m)_m={code:_c,owner_token:_ft};const _fk=(location.hash.match(/k=([0-9a-f]+)/)||[])[1];try{const d=await api('GET','/api/melds/'+_c);let a=d.context_a,b=d.context_b;if(_fk){a=await _dec(a,_fk);if(b)b=await _dec(b,_fk)}if(d.resolved){r.innerHTML=RD(a,b)}else{r.innerHTML=M(a)}}catch(e){const m=e.message||'';if(m.includes('410')||m.includes('expired')){r.innerHTML='<div class="card" style="text-align:center"><h1>Gone</h1><p style="color:var(--dim)">This meld has expired.</p><a href="/" class="btn" style="margin-top:1rem">Create</a></div>'}else{r.innerHTML='<div class="card" style="text-align:center"><h1>Not found</h1><p style="color:var(--dim)">This meld doesn\\'t exist.</p><a href="/" class="btn" style="margin-top:1rem">Create</a></div>'}}}
...
<p>meld is AGPL open source; the hosted service runs this exact code. You can confirm the ciphertext behavior yourself: create an E2E meld, then <code>GET /api/melds/&lt;code&gt;</code> — you will see <code>meld1:…</code>, not your text.</p>

<p style="color:#5a5a72;font-size:.8125rem;margin-top:2rem">Full document: <a href="https://github.com/lemonaide152/meld/blob/main/TRUST.md">TRUST.md in the repository</a>.</p>
</body></html>""")
